...
**`androidx.security:security-crypto` (EncryptedSharedPreferences → Tink) is an untested exposure — no fleet app has proven it under R8 yet.** Tink resolves key templates and protobuf types reflectively, which is the same shape as the serialization trap: strip it and the **token store fails at runtime**, so sign-in breaks while everything compiles and launches. allotmint uses it for its bearer token; as of 2026-07-17 its minified build has **not** yet been smoke-tested. Treat this as unproven until someone signs in on a minified build — and when they do, record the required keeps here.
